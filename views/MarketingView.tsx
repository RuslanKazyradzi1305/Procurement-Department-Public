          ...
          <div ref={docRef} className="bg-white shadow-2xl min-h-[1123px] w-full p-[20mm] mx-auto print:shadow-none print:p-0"
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', border: '1pt solid black', fontSize: '9pt' }}>
              <thead>
                ...
                <tr>
                  <th style={{ border: '1pt solid black', padding: '3pt', fontSize: '8pt', fontWeight: 'bold' }}>{sup1}</th>
                  <th style={{ border: '1pt solid black', padding: '3pt', fontSize: '8pt', fontWeight: 'bold' }}>{sup2}</th>
                  <th style={{ border: '1pt solid black', padding: '3pt', fontSize: '8pt', fontWeight: 'bold' }}>{sup3}</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row, index) => (
                  <tr key={row.id}>
                    <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{index + 1}</td>
                    <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'left', lineHeight: '1.1' }}>{row.name || '—'}</td>
                    <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{row.unit || '—'}</td>
                    <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{row.quantity}</td>
                    <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{row.cost1 > 0 ? formatCurrency(row.cost1) : '—'}</td>
                    <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{row.cost2 > 0 ? formatCurrency(row.cost2) : '—'}</td>
                    <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{row.cost3 > 0 ? formatCurrency(row.cost3) : '—'}</td>
                    <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center', fontWeight: 'bold' }}>
                ...
                <tr style={{ fontWeight: 'bold' }}>
                  <td colSpan={4} style={{ border: '1pt solid black', padding: '4pt', textAlign: 'right', fontStyle: 'italic' }}>Итого (без учета НДС):</td>
                  <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{formatCurrency(rows.reduce((a, b) => a + (b.cost1 * b.quantity), 0))}</td>
                  <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{formatCurrency(rows.reduce((a, b) => a + (b.cost2 * b.quantity), 0))}</td>
                  <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{formatCurrency(rows.reduce((a, b) => a + (b.cost3 * b.quantity), 0))}</td>
                  <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center', backgroundColor: '#f9fafb' }} className="print:bg-transparent">{formatCurrency(totalBase)}</td>
                </tr>
                {showVAT && (
                  <>
                    <tr style={{ fontWeight: 'bold' }}>
                      <td colSpan={7} style={{ border: '1pt solid black', padding: '4pt', textAlign: 'right', fontStyle: 'italic' }}>НДС (16%):</td>
                      <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{formatCurrency(totalVAT)}</td>
                    </tr>
                    <tr style={{ fontWeight: 'bold', fontSize: '11pt' }}>
                      <td colSpan={7} style={{ border: '1pt solid black', padding: '4pt', textAlign: 'right', fontStyle: 'italic', textTransform: 'uppercase' }}>Всего с учетом НДС:</td>
                      <td style={{ border: '1pt solid black', padding: '4pt', textAlign: 'center' }}>{formatCurrency(totalWithVAT)}</td>
                    </tr>
                  </>
                )}
              </tbody>
