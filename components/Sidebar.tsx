...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, isOpen, setIsOpen }) => {
  ...
  const menuItems = [
    ...
    { id: AppTab.Protocols, label: t('menu_protocols'), icon: ClipboardList },
    { id: AppTab.ProcurementPlan, label: t('menu_plan'), icon: Calendar },
    { id: AppTab.SuppliersDB, label: t('menu_suppliers_db'), icon: Users },
    { id: AppTab.SuppliersDashboard, label: t('menu_dashboard'), icon: PieChart },
    { id: AppTab.EnsTru, label: t('menu_enstru'), icon: Database },
  ];

  return (
    <aside className={`fixed left-0 top-0 h-screen glass border-r border-slate-200 transition-all duration-500 z-50 ${isOpen ? 'w-64' : 'w-20'}`}>
      ...
      <nav className="mt-8 px-3 space-y-2">
        {menuItems.map((item) => (
          ...
          >
            ...
            {isOpen && <span className="font-semibold whitespace-nowrap overflow-hidden text-sm tracking-tight">{item.label}</span>}
          </button>
        ))}
      </nav>
